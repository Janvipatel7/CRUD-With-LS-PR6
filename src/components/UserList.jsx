const UserList = ({ users, deleteUser, getEditUser }) => {
  ...
  return (
    <div className="container mx-auto px-4 md:px-0 my-12">
      <div className="relative overflow-x-auto shadow-xl rounded-xl border border-gray-200 bg-white p-6">
        <h2 className="text-2xl font-bold text-center text-black mb-4 pt-5">
          Student User List
        </h2>
        {users.length > 0 ? (
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id} className="odd:bg-white even:bg-gray-50 border-b">
                  ...
                  <td className="px-6 py-3 font-medium text-gray-900">{user.gender}</td>
                  <td className="px-6 py-3 font-medium text-gray-900">{user.password}</td>
                  <td className="px-6 py-3 font-medium text-gray-900">
